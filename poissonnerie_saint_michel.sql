...
-- Procédures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_changer_statut_commande` (IN `p_id_commande` BIGINT UNSIGNED, IN `p_nouveau_statut` VARCHAR(30), IN `p_origine` VARCHAR(15), IN `p_id_utilisateur` BIGINT UNSIGNED, IN `p_commentaire` VARCHAR(500))   BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `ai_commandes_historique_initial` AFTER INSERT ON `commandes` FOR EACH ROW BEGIN
  INSERT INTO historique_statuts_commande (
...
--
DELIMITER $$
CREATE TRIGGER `bi_justificatifs_controles` BEFORE INSERT ON `justificatifs_paiement` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `ad_lignes_commande_recalculer_total` AFTER DELETE ON `lignes_commande` FOR EACH ROW BEGIN
  UPDATE commandes
     SET total_produits = (
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `ai_lignes_commande_recalculer_total` AFTER INSERT ON `lignes_commande` FOR EACH ROW BEGIN
  UPDATE commandes
     SET total_produits = (
           SELECT COALESCE(SUM(sous_total), 0.00)
             FROM lignes_commande
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `au_lignes_commande_recalculer_total` AFTER UPDATE ON `lignes_commande` FOR EACH ROW BEGIN
  UPDATE commandes
     SET total_produits = (
           SELECT COALESCE(SUM(sous_total), 0.00)
             FROM lignes_commande
...
--
DELIMITER $$
CREATE TRIGGER `bi_livraisons_controles` BEFORE INSERT ON `livraisons` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `ai_mouvements_stock_synchroniser_produit` AFTER INSERT ON `mouvements_stock` FOR EACH ROW BEGIN
  UPDATE produits
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `bi_mouvements_stock_calculer_solde` BEFORE INSERT ON `mouvements_stock` FOR EACH ROW BEGIN
...
--
CREATE TABLE `vue_produits_stock_bas` (
`id_produit` bigint unsigned
,`libelle` varchar(150)
,`quantite_stock` decimal(14,3)
,`seuil_alerte` decimal(14,3)
,`unite_vente` enum('KG','CARTON','ALVEOLE','UNITE','PAQUET','AUTRE')
);
...
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `vue_recettes_journalieres` (
`jour` date
,`nombre_paiements` bigint
,`recettes` decimal(36,2)
);
...
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `vue_ventes_par_produit` (
`categorie` varchar(100)
,`chiffre_affaires` decimal(36,2)
,`id_produit` bigint unsigned
,`produit` varchar(150)
,`quantite_vendue` decimal(36,3)
);
...
--
-- Structure de la vue `vue_produits_stock_bas`
--
DROP TABLE IF EXISTS `vue_produits_stock_bas`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_produits_stock_bas`  AS SELECT `produits`.`id_produit` AS `id_produit`, `produits`.`libelle` AS `libelle`, `produits`.`quantite_stock` AS `quantite_stock`, `produits`.`seuil_alerte` AS `seuil_alerte`, `produits`.`unite_vente` AS `unite_vente` FROM `produits` WHERE ((`produits`.`est_actif` = true) AND (`produits`.`quantite_stock` <= `produits`.`seuil_alerte`)) ;

-- --------------------------------------------------------

--
-- Structure de la vue `vue_recettes_journalieres`
--
DROP TABLE IF EXISTS `vue_recettes_journalieres`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_recettes_journalieres`  AS SELECT cast(`paiements`.`date_confirmation` as date) AS `jour`, count(0) AS `nombre_paiements`, sum(`paiements`.`montant`) AS `recettes` FROM `paiements` WHERE (`paiements`.`statut_paiement` = 'REUSSI') GROUP BY cast(`paiements`.`date_confirmation` as date) ;

-- --------------------------------------------------------

--
-- Structure de la vue `vue_ventes_par_produit`
--
DROP TABLE IF EXISTS `vue_ventes_par_produit`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_ventes_par_produit`  AS SELECT `p`.`id_produit` AS `id_produit`, `p`.`libelle` AS `produit`, `c`.`libelle` AS `categorie`, sum(`lc`.`quantite`) AS `quantite_vendue`, sum(`lc`.`sous_total`) AS `chiffre_affaires` FROM ((((`lignes_commande` `lc` join `produits` `p` on((`p`.`id_produit` = `lc`.`id_produit`))) join `categories` `c` on((`c`.`id_categorie` = `p`.`id_categorie`))) join `commandes` `co` on((`co`.`id_commande` = `lc`.`id_commande`))) join `paiements` `pa` on((`pa`.`id_commande` = `co`.`id_commande`))) WHERE (`pa`.`statut_paiement` = 'REUSSI') GROUP BY `p`.`id_produit`, `p`.`libelle`, `c`.`libelle` ;
